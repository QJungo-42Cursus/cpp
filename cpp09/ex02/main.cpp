...
{
	struct timeval start, end, end_set, start_vector, end_vector;
	...
	for (std::set<unsigned int>::iterator it = set.begin(); it != set.end(); it++)
		std::cout << *it << " ";
	std::cout << std::endl;
	gettimeofday(&end_set, NULL);
	double set_time_us = (end_set.tv_sec - end.tv_sec) * 1000000.0 + (end_set.tv_usec - end.tv_usec) + common_time_us;
	std::cout << "Time for set: " << std::fixed << std::setprecision(3) << set_time_us / 1000.0 << " ms" << std::endl;
