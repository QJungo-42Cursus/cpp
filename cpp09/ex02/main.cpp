...
int main(int argc, char **argv) {
    ...
    std::cout << "Time for set: \t\t" << std::fixed << std::setprecision(3) << common_time_us / 1000.0 << " ms"
              << std::endl;
    ...
    double vec_time_us =
            (end_vector.tv_sec - start_vector.tv_sec) * 1000000.0 + (end_vector.tv_usec - start_vector.tv_usec) +
            common_time_us;
    std::cout << "Time for vector: \t" << std::fixed << std::setprecision(3) << vec_time_us / 1000.0 << " ms"
              << std::endl;
}
